# Next-generation prospect analysis specification

This file is the authoritative Priority 1–5 specification for `analyzer_nextgen`. The next-generation program loads this document at runtime, so the scoring, judgement, evidence, and explanation rules below are not duplicated in the Python modules. The legacy `analyzer\analysis.md` remains separate.

Run the next-generation analyzer as a module:
...
To use Gemini research through the same LiteLLM proxy key, change only the research settings:

```text
python -m analyzer_nextgen.main `
  --research-provider gemini `
  --research-model gemini/gemini-2.5-flash
```

Profile analysis remains controlled independently by `--model`. The proxy API key is shared; the LiteLLM gateway must have the selected upstream provider credentials configured.
...
| `--refresh-research` | Research the companies in the current run again while preserving cached reports and timestamps for all other companies. | Useful after changing `--research-model`; failed searches are not written into the cache. |
...
| `--max-error-rounds N` | After a complete analysis pass, retry only rows saved with `Error` until none remain or this limit is reached. | `10` |
| `--max-research-rounds N` | After all normal batches finish, retry companies whose research remained unavailable, then reanalyze their affected rows. | `3` |
| `--status-report PATH` | Write the final research/error status report. | `<output>.status.json` |
...
The Python analyzer processes each batch in two stages: up to the configured worker count concurrently, `gpt-5.2` calls `/responses` with `web_search_preview` to research each unique company, then `gpt-5.6-luna` analyzes the batch profiles using that research. Empty responses, generic refusals, failure messages, and reports without a source URL are rejected and retried like API exceptions. Only validated research is persisted in the cache. If research remains unavailable after its per-request retries, Priority 1 receives a temporary unavailable-research placeholder; that placeholder is not cached. After all normal batches finish, unavailable companies are retried in complete research rounds, and rows affected by recovered companies are reanalyzed. Only then are faulty profile-analysis rows retried in complete analysis rounds until none remain or the configured limit is reached. A final JSON status report lists unresolved research companies, affected row IDs, and faulty row IDs. Previously valid results are preserved if a later retry fails. Research is persisted in `company-research-cache.json`, which is ignored by Git, so later runs reuse existing results instead of searching the same company again. Use `--refresh-research` after changing the research model to refresh only companies in the current run while preserving unrelated cache entries. Use `--research-model` to change the research model, `--research-cache` to choose another cache file, or `--no-web-search` to disable searching intentionally.
...
Company research is performed once per normalized company name, so multiple profiles at the same company reuse the same research. Research and profile analysis both use the configured worker count, but each batch waits for its company research to finish before its profile analysis begins. With `--refresh-research`, only companies in the current run are re-researched and unrelated cache entries and their `researched_at` timestamps remain intact. Console output is forced to UTF-8 for European names and accents.
