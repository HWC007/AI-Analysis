...
PRIORITY 2 SCOPE CLARIFICATION (MANDATORY ROLE EXCLUSIONS): Exclude general sales, technical-sales, machine setters/operators, technicians, quality engineers, and production leaders by default. These roles count for Priority 2 ONLY when `Current_Position_Description` explicitly states involvement in mold tooling, mold trials, mold modifications, plastic-part development, or injection-molding engineering. Plastic-part development may involve different injection-molding techniques or variants; do not require the description to name a particular injection process. Do not count work based on blow molding, thermoforming, or rotational molding. Evidence from the job title, headline, company identity, `Skills`, `about`, previous roles, or web research cannot substitute for that explicit current-position description. Machine operation, parameter setting, production supervision, inspection, quality control, or selling without explicit mold/tooling evidence in `Current_Position_Description` is insufficient. Pure mold-maintenance or mold-repair work does not qualify by itself.
PRIORITY 2 — CURRENT ROLE: Analyze Current_Position, Current_Position_Description, headline, and tenure. Apply the mandatory role exclusions above before considering any positive signal. For Priority 2, count injection-molding work and its relevant techniques or variants when explicitly supported; do not count blow molding, thermoforming, rotational molding, or work based exclusively on those processes. Service providers/engineering firms permit high inference for technical/design roles that are not in the excluded categories. For a confirmed injection-molding or tooling provider, roles such as Geschäftsführer/Managing Director, technical director, engineering lead, construction/development lead, tooling lead, process engineer, automation lead, or comparable technical leader may qualify when the profile connects them to the company's technical, production, tooling, or digitalization work. For OEMs, require plastic-specific evidence such as plastic-part design, lightweight components, enclosures, tooling, injection molding, mold trials, Moldflow, or Cadmould.
