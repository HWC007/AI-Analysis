...
PRIORITY 1 — COMPANY QUALIFICATION: Analyze Current_Company. Mark Priority 1 True only when reliable evidence supports at least one of these definitions: (1) a tooling/molding service provider that designs or builds injection molds, produces molded parts for customers, provides mold-tooling engineering, performs mold trials, or performs mold modifications; (2) a molding-technology provider that designs or manufactures hot-runner systems, injection-mold cooling channels, conformal-cooling solutions, or other systems integrated directly into injection molds; (3) a plastics simulation/engineering provider offering plastic-part engineering, injection-molding engineering, or Moldflow, Cadmould, Moldex3D, or comparable molding simulation; or (4) a product manufacturer/OEM whose physical products substantially use plastic components, such as automotive products, medical devices, electronics, appliances, toys, or industrial equipment.
TARGET DEFINITION: This workflow targets injection-molding manufacturers, mold/tooling builders, plastics engineering providers, and relevant technical stakeholders or decision-makers at those companies. Moldflow, Cadmould, or Moldex3D is NOT required for company or personal qualification. Do not mark a clearly qualifying company or relevant technical person false merely because those named software tools are absent. Exclude companies whose main business is only injection-molding machines; dryers, robots, loaders, chillers, or other peripheral/auxiliary equipment; general factory automation unrelated to mold tooling; plastics-material supply without molding, tooling, molded-part, or plastics-engineering services; or mold maintenance, mold repair, or mold servicing without broader mold design, tooling engineering, mold trials, modifications, or molded-part production.
...
PRIORITY 2 — ORGANIZED CURRENT-ROLE RULES: Analyze Current_Position, Current_Position_Description, headline, and Current_Tenure. Priority 2 concerns the individual's current role, not merely the company. Company qualification under Priority 1 is context only and must not by itself make Priority 2 True.
...
PRIORITY 4 — COMPETITION SOFTWARE: Search case-insensitively in the supplied LinkedIn/profile fields for Moldflow, Cadmould, or the exact phrase `SolidWorks Plastics` or `SolidWorks plastic`. A standalone `SolidWorks` or `Solidworks` mention does NOT satisfy Priority 4. Check Skills separately from all other sections. p4_in_skills_only is true only if a qualifying term appears in Skills and nowhere else. p4_in_other_sections is true if a qualifying term appears outside Skills; if it appears in both locations, use the other-sections weighting. Priority 4 is informational only and never affects AI_Judgement.
...
RESPONSE CONSISTENCY: Keep the original JSON response fields, including the five priority booleans, `judgement`, the Priority 4 location flags, and one complete `explanation` string. In that explanation string, write one detailed evidence-based section for each priority and finish each section with exactly one separate final line in this exact format: `Conclusion: True` or `Conclusion: False`. The conclusion must follow that priority's explicit conclusion condition and summarize the evidence immediately above it. A section that says qualifying evidence is absent must end with `Conclusion: False`; a section that says qualifying evidence is present must end with `Conclusion: True`. Do not place another `Conclusion:` line inside the same priority section, and do not write prose that contradicts the final conclusion. The Python parser will use the five terminal conclusion lines inside the explanation as the authoritative priority decisions and will ignore the independently supplied priority booleans.
